...
type
  ...
  public
    ...
    procedure Insert(s:String); //insert s into alphabteical order
    function Delete(s:string):boolean; //remove s if it exists, return false if not found
    function GetItem(n:integer):string; //return nth item in list
...
function TLList.GetItem(n: integer): string;
var
  current:PLList;
  nodenum:integer;
begin
  if (FStart=nil) or (n>FLength) or (n<1) then result:='' //deal with impossible requests
  else
    if n=1 then result:=FStart^.data      //special case:first node required
    else
      begin
        current:=Fstart;                              //general case
        for nodenum:=1 to n-1 do                  //fill this in yourself
        current:=current^.next;           //start at the beginning
        result:=current^.data;            //follow the pointers to the nth node
      end;
end;
